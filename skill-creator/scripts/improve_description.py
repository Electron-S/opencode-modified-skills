...
by calling `opencode run` as a subprocess instead of `claude -p`.
"""
...
def _call_opencode(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run `opencode run --format json` and return the full text response.

    Collects all text events from the JSON stream and concatenates them.
    Prompt is passed as the query argument directly.
    """
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # Safety net: if the model exceeded the 1024-char hard limit, ask for a shorter rewrite.
...
def main():
    ...
    parser.add_argument("--model", required=True, help="Model for improvement, e.g. provider/model-name")
